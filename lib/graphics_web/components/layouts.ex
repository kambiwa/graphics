  defmodule GraphicsWeb.Layouts do
    ...
    attr :current_scope, :map, default: nil
      ...
      def public_nav(assigns) do
        ...
        <nav id="navbar" style="position:fixed;top:0;left:0;right:0;z-index:100;background:rgba(237,236,234,0.88);backdrop-filter:blur(14px);border-bottom:0.5px solid rgba(26,26,24,0.12);display:flex;align-items:center;justify-content:space-between;padding:0 4rem;height:64px;">
          ...
          <a href="/" style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;letter-spacing:0.15em;color:#1a1a18;text-decoration:none;">
            JAGUARGRAPHICS
          </a>

          <%!-- Desktop links --%>
          <div id="nav-desktop" style="display:flex;gap:2.5rem;align-items:center;">
            <a href="#about"     class="nav-link">About</a>
            <a href="#services"  class="nav-link">Services</a>
            <a href="#portfolio" class="nav-link">Portfolio</a>
            <a href="#pricing"   class="nav-link">Pricing</a>
            <%= if @current_scope do %>
              <a href={~p"/users/settings"} class="nav-link">Settings</a>
              <.link href={~p"/users/log-out"} method="delete" class="nav-link">Log out</.link>
            <% else %>
              <a href="/users/log-in" class="nav-link">Login</a>
            <% end %>
          ...
          <button
            ...
            style="display:none;flex-direction:column;gap:5px;background:none;border:none;cursor:pointer;padding:4px;"
          >
      ...
      <div
        id="mobile-drawer"
        style="display:none;position:fixed;top:64px;left:0;right:0;z-index:99;background:#1a1a18;border-bottom:0.5px solid rgba(255,255,255,0.08);overflow:hidden;max-height:0;transition:max-height 0.35s cubic-bezier(0.22,1,0.36,1);"
      >
        <div style="padding:1.2rem 2rem 1.8rem;display:flex;flex-direction:column;gap:0;">
          <a href="#about"     class="mob-link" onclick="closeMobileNav()">About</a>
          <a href="#services"  class="mob-link" onclick="closeMobileNav()">Services</a>
          <a href="#portfolio" class="mob-link" onclick="closeMobileNav()">Portfolio</a>
          <a href="#pricing"   class="mob-link" onclick="closeMobileNav()">Pricing</a>
          <%= if @current_scope do %>
            <a href={~p"/users/settings"} class="mob-link">Settings</a>
            <.link href={~p"/users/log-out"} method="delete" class="mob-link">Log out</.link>
          <% else %>
            <a href="/users/log-in" class="mob-link">Login</a>
          <% end %>
          <a href="#booking" onclick="closeMobileNav()" style="margin-top:1.2rem;display:block;background:#E05C3A;color:#FAF9F7;text-align:center;padding:0.9rem;border-radius:999px;font-size:0.82rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;text-decoration:none;">
            Book Now
          </a>
        </div>
      </div>
    ...
    def public_footer(assigns) do
      ...
      <footer>
        ...
        <p>Powered by Mweembo kambiwa</p>
        <div class="footer-links">
        <p>jaguargraphics@<%= Date.utc_today().year %></p>
        </div>
      </footer>
      """
    end
